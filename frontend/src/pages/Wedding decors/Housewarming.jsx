...
const Housewarming = () => {
  ...
  useEffect(() => {
    loadHousewarmingData();
  }, []);

  const loadHousewarmingData = async () => {
    try {
      setLoading(true);
      
      // Get all categories
      const categoriesData = await housewarmingService.getAllCategories();
      ...
      for (const categoryName of categoryNames) {
        const images = await housewarmingService.getCategoryImages(categoryName);
      ...
      console.error("Error loading housewarming data:", error);
    } finally {
      setLoading(false);
    }
  };
      ...
      <Container className="py-4">
        ...
        {loading ? (
          <div className="text-center py-5">
            ...
            <p className="mt-2">Loading housewarming decorations...</p>
          </div>
        ) : (
          <>
        ...
        {loading ? null : filteredImages.length > 0 ? (
              ...
              <Col key={`${image.fileName || image.name}_${index}`} xs={12} sm={6} md={4} className="gallery-item mb-4">
                ...
                >
                  <img 
                    ...
                    alt={image.name || `Housewarming decoration ${index + 1}`} 
          ...
          <div className="text-center py-5">
            <p style={{ fontFamily: 'Poppins, sans-serif', color: '#777' }}>
              {categories.length === 0 
                ? "No housewarming decorations available yet. Please check back soon!"
