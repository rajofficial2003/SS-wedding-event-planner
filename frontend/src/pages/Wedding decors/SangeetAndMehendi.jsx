...
const SangeetAndMehendi = () => {
  ...
  useEffect(() => {
    loadSangeetMehandiData()
  }, [])

  const loadSangeetMehandiData = async () => {
    try {
      setLoading(true)

      // Get all categories
      const categoriesData = await sangeetmehandiService.getAllCategories()
      ...
      for (const categoryName of categoryNames) {
        const images = await sangeetmehandiService.getCategoryImages(categoryName)
      ...
      console.error("Error loading sangeet mehandi data:", error)
    } finally {
      setLoading(false)
    }
  }
    ...
    <div className="sangeet-mehendi-page">
      ...
      <Container className="py-4">
        <p className="mb-4" style={{ lineHeight: "1.8" }}>
          To elevate your Mehendi function, the bride's has to be super amazing and beautiful just like you. Mehendi
          literally fall on hands help you with some great ideas and the market for your pre-wedding functions. A chance
          and creative space can create for Mehendi. Can coordinate plays, a contest for Sangeet night like, all
          relatives friends and Dancers, don't want them to be left out. We are here to help you with all your needs.
        </p>
        ...
        {loading ? (
          <div className="text-center py-5">
            ...
            <p className="mt-2">Loading sangeet and mehandi decorations...</p>
          </div>
        ) : (
          <>
        ...
        {loading ? null : filteredImages.length > 0 ? (
              ...
              <Col key={`${image.fileName || image.name}_${index}`} xs={12} sm={6} md={4} className="gallery-item mb-4">
                ...
                >
                  <img
                    ...
                    alt={image.name || `Sangeet mehandi decoration ${index + 1}`}
          ...
          <div className="text-center py-5">
            <p style={{ fontFamily: "Poppins, sans-serif", color: "#777" }}>
              {categories.length === 0
                ? "No sangeet and mehandi decorations available yet. Please check back soon!"
